...
export function getProjectById(projectId: string, supabase: any) {
  ...
  return withRetry(async () => {
    const { data: project, error: projectError } = await supabase
      .from("project")
      .select(
        "project_id, allowed_urls, direction, language, language_code, icon_url, client_name, client_description, highlight_color, show_ad, enabled, input_text_placeholders, display_mode, display_position, article_class, widget_container_class, override_mobile_container_selector, disclaimer_text, widget_mode, ask_concent",
      )
